...
class SetupWizard:
    ...
    def _new_setup_flow(self):
        ...
        mode_map = {"1": "ide_only", "2": "ide_web"}
        selected = mode_map.get(choice, "ide_only")
        
        self.answers["action"] = "new_setup"
        self.answers["mode"] = selected
        
        # Configure Web if needed
        if selected == "ide_web":
            self._configure_web()
...
"""
    ...
    def _generate_files(self):
        ...
        if action == "add_web":
            self._update_add_web()
        elif action == "remove_web":
            self._update_remove_web()
        elif action == "edit_settings":
...
"""
    ...
    def _create_new_config(self):
        ...
        config = {
            ...
            "mode": {
                "primary": "ide" if self.answers["mode"] == "ide_only" else "hybrid",
                "ide": {"enabled": True},
                "telegram": {
                    "enabled": False,
                    "bot_token": "",
                ...
                "web": {
                    "enabled": self.answers["mode"] == "ide_web",
                    "port": 8082
                }
            },
            "provider": {
