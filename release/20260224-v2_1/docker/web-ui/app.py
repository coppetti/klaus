...
# Try to import context compactor
try:
    ...
    print("⚠️  Context compactor not available")
            ...
            <header class="px-6 py-3 border-b border-gray-200 bg-white">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center bg-gray-100 rounded-lg p-1">
                        <button onclick="switchTab('chat')" id="tab-chat" class="px-4 py-1.5 rounded-md text-sm font-medium bg-white text-gray-900 shadow-sm transition-all">
                            <i class="fas fa-comments mr-2"></i>Chat
                        </button>
                        <button onclick="switchTab('graph')" id="tab-graph" class="px-4 py-1.5 rounded-md text-sm font-medium text-gray-600 hover:text-gray-900 transition-all">
                            <i class="fas fa-project-diagram mr-2"></i>Graph
                        </button>
                        <button onclick="switchTab('episodic')" id="tab-episodic" class="px-4 py-1.5 rounded-md text-sm font-medium text-gray-600 hover:text-gray-900 transition-all">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div class="sidebar-card">
                    ...
                    <div class="space-y-3">
                        <!-- Bot Token -->
                        <div>
                            <label class="text-xs text-gray-500 block mb-1">Bot Token <span class="text-red-500">*</span></label>
                            ...
                            <p class="text-[10px] text-gray-400 mt-1">Get from @BotFather</p>
                        </div>
                        
                        <!-- Chat ID -->
                        <div>
                            <label class="text-xs text-gray-500 block mb-1">Your Telegram ID (Chat ID) <span class="text-red-500">*</span></label>
                            ...
                            <p class="text-[10px] text-gray-400 mt-1">
                                <a href="https://t.me/userinfobot" target="_blank" class="text-blue-600 hover:underline">Get your ID from @userinfobot</a>
                            </p>
                        </div>
                        ...
                        <div class="flex gap-2 pt-2">
                            ...
                            <button onclick="launchTelegramBot()" id="telegram-launch-btn" class="btn btn-primary flex-1 text-xs py-1.5 bg-blue-600 hover:bg-blue-700">
                                <i class="fas fa-play mr-1"></i>Launch Bot
                            </button>
                        </div>
                        
                        <!-- Last Status -->
                        <div id="telegram-last-status" class="hidden text-[10px] text-gray-400 pt-1 border-t border-gray-100 mt-2">
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        async function loadSettings() {{
            try {{
                ...
                // Only update UI if settings elements exist (not in sidebar anymore)
        ...
        async function updateSettings() {{
            const providerSelect = document.getElementById('setting-provider');
            if (!providerSelect) return; // Settings not in sidebar anymore
...
async def chat_with_provider(provider: str, model: str, messages: list, system: str, temperature: float, max_tokens: int):
    ...
    try:
        if provider == "kimi":
            ...
            print(f"DEBUG: Calling Kimi API with model={model}, messages={len(messages)}, has_system={bool(system)}")
            
            # Stream the response (required by Anthropic SDK for long requests)
            ...
            # For OpenAI, prepend system message to messages list
            ...
            # For OpenRouter, prepend system message to messages list
            ...
            if not base_url:
                raise Exception("Custom provider requires a base URL. Configure it in settings.")
            ...
            try:
                ...
                raise Exception(f"Failed to initialize custom client: {e}")
            
            # For custom providers, prepend system message to messages list
        ...
        print(f"ERROR in chat_with_provider: {e}")
        import traceback
        traceback.print_exc()
        raise
...
<body class="bg-gray-50 h-screen">
    ...
    <script>
        ...
        async function loadGraph() {
            try {
                const [kgRes, statsRes] = await Promise.all([
                    fetch('/api/cognitive-memory/knowledge-graph'),
                ...
                document.getElementById('stats').innerHTML = `
                    <span class="font-medium">${stats.episodic_count || 0}</span> episodes
                    <span class="mx-2">|</span>
                    <span class="font-medium">${stats.semantic_entities || 0}</span> entities
                `;
                
                allEntities = kgData.entities || [];
                allRelationships = kgData.relationships || [];
                
                renderEntityList(allEntities);
                buildGraph(allEntities, allRelationships);
            } catch (error) {
                console.error('Failed to load:', error);
                document.getElementById('graph-container').innerHTML = '<div class="flex items-center justify-center h-full text-red-500">Failed to load</div>';
            }
        }
        ...
        function renderEntityList(entities) {
            ...
            if (!entities || entities.length === 0) {
                container.innerHTML = `<div class="text-center text-gray-400 py-8">${isFiltering ? "No entities match your search" : "No entities"}</div>`;
                return;
            }
            container.innerHTML = entities.map(e => `
                <div class="p-2 bg-gray-50 rounded border border-gray-200 hover:border-pink-300 cursor-pointer" onclick="highlight('${e.id}')">
                    <div class="flex items-center gap-2">
                        <span class="w-3 h-3 rounded-full" style="background: ${colors[e.type]?.background || '#999'}"></span>
        ...
        function buildGraph(entities, relationships) {
            const nodes = entities.map(e => ({
                id: e.id, label: e.name, group: e.type,
                color: colors[e.type] || { background: '#999', border: '#666' },
                shape: 'dot', size: 25
            }));
            const edges = relationships.map(r => ({
                from: r.source_id, to: r.target_id,
                label: r.type, arrows: 'to', color: { color: '#cbd5e1' }
            }));
        ...
        function highlight(id) { if (network) { network.selectNodes([id]); network.focus(id, { scale: 1.5 }); } }
        
        // Store original graph data for reset
        let originalEntities = [];
        let originalRelationships = [];
        
        function showMemoryInGraph(memoryId, userMessage) {
            // Get entities related to this memory
            const relatedEntities = new Set();
            
            // Find entities mentioned in the memory's message or entities involved
            allEntities.forEach(e => {
                // Check if entity name appears in the message
                if (userMessage && userMessage.toLowerCase().includes(e.name.toLowerCase())) {
                    relatedEntities.add(e.id);
                }
                // Also check relationships - find entities connected to memory-related entities
            });
            
            // Find all relationships where at least one entity is related to the memory
            const relatedRelations = allRelationships.filter(r => {
                const source = allEntities.find(e => e.id === r.source_id);
                const target = allEntities.find(e => e.id === r.target_id);
                
                // Check if source or target name appears in message
                const sourceMatch = source && userMessage && userMessage.toLowerCase().includes(source.name.toLowerCase());
                const targetMatch = target && userMessage && userMessage.toLowerCase().includes(target.name.toLowerCase());
                
                return sourceMatch || targetMatch;
            });
            
            // Add connected entities from relationships
            relatedRelations.forEach(r => {
                relatedEntities.add(r.source_id);
                relatedEntities.add(r.target_id);
            });
            
            // If no specific entities found, show a message
            if (relatedEntities.size === 0) {
                alert('No graph entities directly linked to this memory. Try searching for: ' + userMessage.substring(0, 50) + '...');
                return;
            }
            
            // Filter entities
            const filteredEntities = allEntities.filter(e => relatedEntities.has(e.id));
            
            // Build subgraph
            buildGraph(filteredEntities, relatedRelations);
...
async def health():
    ...
    return {
        ...
        "features": ["settings", "sessions", "hybrid_memory", "multi_provider", "web_search", "templates", "admin_panel"],
...
async def save_telegram_settings(request: Request):
    ...
    try:
        ...
        # Try to update init.yaml (may be read-only in Docker)
        try:
            ...
            if init_path.exists():
                ...
                if "telegram" not in config["mode"]:
                    config["mode"]["telegram"] = dict()
                
                config["mode"]["telegram"]["enabled"] = enabled
                if token:
                    config["mode"]["telegram"]["token"] = token
                if allowed_chat_ids:
                    config["mode"]["telegram"]["allowed_chat_ids"] = allowed_chat_ids
                
                with open(init_path, 'w') as f:
                    yaml.dump(config, f, default_flow_style=False)
        except (OSError, IOError) as e:
            # init.yaml is read-only, that's OK - we'll use env vars
            print(f"Note: init.yaml is read-only, using environment variables: {e}")
        
        # Save to environment for immediate use (primary storage)
        if token:
...
async def get_settings():
    ...
    return {
        ...
        "telegram_enabled": settings.telegram_enabled if hasattr(settings, 'telegram_enabled') else False,
    }


# Get Provider-specific Settings API
@app.get("/api/settings/provider/{provider}")
async def get_provider_settings(provider: str):
    """Get settings for a specific provider (without exposing API keys)."""
    provider_env_vars = {
        "kimi": "KIMI_API_KEY",
    ...
    url_env_var = provider_url_env_vars.get(provider)
    
    api_key_exists = os.getenv(env_var) is not None if env_var else False
    configured_model = os.getenv(model_env_var) if model_env_var else None
    base_url = os.getenv(url_env_var) if url_env_var else None
    
    # If no specific model env var, use the current settings model if provider matches
    if not configured_model and settings.provider == provider:
