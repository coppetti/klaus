...
| Hybrid Memory (Graph) | ✅ Stable | Embedded Semantic Engine, Durable Sync, and strict anti-noise caps added |
...
**File:** `core/hybrid_memory.py:416-560`
...
### Warning #2: Web UI File Upload No Virus Scan
...
### Cognitive Hardening & Kuzu Semantic Fixes (2026-02-25)
**Status:** ✅ IMPLEMENTED  
**Files:** `core/hybrid_memory.py`, `core/memory_relevance_gate.py`, `tests/unit/test_hybrid_memory.py`

**Features:**
- **Relevance Gate**: Auto-filters low-value "ok/thanks" messages from permanent SQLite/Graph storage.
- **Durable Graph Sync**: Replaced in-memory queue with SQLite queue. Guaranteed graph synchronisation even after container crash.
- **Dense Offline Embeddings**: Embedded `sentence-transformers` models running on CPU to score text similarity locally without APIs.
- **Strict Edge Filters (CASTLE Logic)**: Graph now connects `FLOWS_INTO` naturally and limits semantic/topic connections to a max 3 strict thresholds (>0.65 Cosine Similarity) to eliminate orphaned nodes and visual noise.
